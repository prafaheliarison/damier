                        ...
                        <div class="p-6">
                            <form action="" method="post">
								...
								<div class="form-group mb-3">
									<label for="exampleInputEmail1">Ligne</label> <input type="number" class="form-control" name="line" required="required" value="{{ !empty($line) ? $line : '' }}">
								</div>
								<div class="form-group mb-3">
									<label for="exampleInputPassword1">Colonne</label> <input type="number" class="form-control" name="column" required="required" value="{{ !empty($column) ? $column : '' }}">
								</div>
								<button type="submit" class="btn btn-primary">OK</button>
							</form>
                        </div>

						@if (!empty($line) && !empty($column))
                                ...
                                <div class="mt-2 text-gray-600 dark:text-gray-400 text-sm">
                                    <table class="damier-table" id="myTable">
                                    	...
                                    	<tr>
                                    		@for ($j = 0; $j <= $column; $j++)
                                    		<td>
                                    			<div class="{{ ($i + $j) % 2 == 0 ? 'black' : 'white' }}"></div>
                                    		</td>
                                    		@endfor
                                    	</tr>
										@endfor
